...
<div class="wrapper">
  ...
  <div class="content-wrapper">
	...
	<?php 
	$query=$db->prepare("SELECT * FROM services_bg WHERE id=:id");
	$query->execute(array("id" => 0));
	$get_service_pic=$query->fetch(PDO::FETCH_ASSOC);
	?>
      <!-- Default box -->
      <div class="box">
        <div class="box-body">
			<form action="assets/process.php" method="post" enctype="multipart/form-data">
			<!-- Service Page Picture -->
				<div class="form-group">
					<label>Sayfa Kapak Resmi</label>
					<br>
					<img src="../<?php echo $get_service_pic["pic_url"]; ?>" width="800px";>
				</div>
				<div class="form-group">
					<input type="file" name="services_bg" value="<?php echo $get_service_pic["pic_url"]; ?>" required>
				</div>
				<div class="form-group">
					<input type="hidden" name="unlink" value="<?php echo $get_service_pic["pic_url"]; ?>">
					<button type="submit" class="btn btn-success" name="service_page_picture_update"><i class="fa fa-refresh"></i> Resmi Güncelle</button>
				</div>
			</form>
		</div>
		<!-- /.box-body -->
